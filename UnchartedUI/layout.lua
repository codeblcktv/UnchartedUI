...
-- Custom oUF tags
-- -------------------------------------------------------
-- Health percent — hidden at full health
oUF.Tags.Events["unchartedui:hp"] = "UNIT_HEALTH UNIT_MAXHEALTH"
oUF.Tags.Methods["unchartedui:hp"] = function(unit)
    -- CurveConstants.ScaleTo100 scales the secret value to 0-100 range
    -- We then pass it directly to string.format — no arithmetic needed
    -- string.format with %d accepts secret numbers (Blizzard relaxed this)
    if not CurveConstants or not CurveConstants.ScaleTo100 then
        return "" -- CurveConstants not available yet
    end
